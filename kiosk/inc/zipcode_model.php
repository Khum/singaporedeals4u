...
class zipcode_model {
   ...
   function get_distance_coords($userlatitude, $userlongitude, $zip2) {
      // added to get distance for mobile searches
      // returns the distance between to zip codes.  If there is an error, the 
      // function will return false and set the $last_error variable.
      ...
      $miles = $this->calculate_mileage($userlatitude, $details2[0], $userlongitude, $details2[1]);
   ...
   function get_zip_details($zip) {
      ...
      $sql = "SELECT lattitude AS lattitude, longitude AS longitude
              FROM zip 
              WHERE zip='$zip'";
   ...
   function get_zip_point($zip) {
      ...
      $sql = "SELECT lattitude, longitude from zip WHERE zip='$zip'";
      $r = mysql_query($sql);
      if (!$r) {
         $this->last_error = mysql_error();
         return false;
   ...
   function get_zips_in_range($zip, $range, $sort=1, $include_base="") {
       // NOTE zee : updated only send zip code in an array
      ...
      $sql = "SELECT zip, lattitude, longitude FROM zip ";
      if ($include_base=="") $sql .= "WHERE zip <> '$zip' AND ";
      else $sql .= "WHERE "; 
      $sql .= "lattitude BETWEEN '$min_lat' AND '$max_lat' 
      ...
      if (!$r) {    // sql error
         ...
         while ($row = mysql_fetch_row($r)) {
            ...
            if ($dist <= $range) {
			   ...
			   //$return[] = str_pad($row[0], 5, "0", STR_PAD_LEFT);
            }
         }
         mysql_free_result($r);
      }
     ...
     function get_zips_in_range_coords($userlatitude, $userlongitude, $range, $sort=1) {
       // NOTE zee : updated only send zip code in an array
       // added by Glenn to search by latitude and longitude for mobile searches
      ...
      // only zips in that range.
      $lat_range = $range/69.172;
      $lon_range = abs($range/(cos($userlatitude) * 69.172));
      $min_lat = number_format($userlatitude - $lat_range, "4", ".", "");
      $max_lat = number_format($userlatitude + $lat_range, "4", ".", "");
      $min_lon = number_format($userlongitude - $lon_range, "4", ".", "");
      $max_lon = number_format($userlongitude + $lon_range, "4", ".", "");

      $return = array();    // declared here for scope

      $sql = "SELECT zip, lattitude, longitude FROM zip WHERE lattitude BETWEEN '$min_lat' AND '$max_lat' AND longitude BETWEEN '$min_lon' AND '$max_lon'";
      ...
      if (!$r) {    // sql error
         ...
         while ($row = mysql_fetch_row($r)) {
            ...
            $dist = $this->calculate_mileage($userlatitude,$row[1],$userlongitude,$row[2]);
            if ($this->units == _UNIT_KILOMETERS) $dist = $dist * _M2KM_FACTOR;
            if ($dist <= $range) {
               $return[str_pad($row[0], 5, "0", STR_PAD_LEFT)] = round($dist, $this->decimals);
			   //$return[] = str_pad($row[0], 5, "0", STR_PAD_LEFT);
