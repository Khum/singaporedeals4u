	...
	function navigation($separator = " | ", $css_current = "", $back_forward = false ) {
		...
		if ($curr_pages <= $all_pages && $curr_pages >= 0) {
			...
			if($all_pages >= 1) {
				$forward = $curr_pages + 1;
				$backward = $curr_pages - 1;
				$navi_string = "<li>".(($curr_pages > 0) ? "<a href=\"".$_SERVER['PHP_SELF']."?".$var."=".$backward.$this->rebuild_qs($var)."\" class=\"btn btn-sm btn-default\">".$this->str_backward."</a>" : '<a class="btn btn-sm btn-default" href="javascript:{};">'.$this->str_backward."</a>")."</li>";
				if (!$back_forward) {
					for($a = $start + 1; $a <= $end; $a++){
						$theNext = $a - 1; // because a array start with 0
						if ($theNext != $curr_pages) {
							$navi_string .= "<li><a href=\"".$_SERVER['PHP_SELF']."?".$var."=".$theNext.$this->rebuild_qs($var)."\" class=\"btn btn-sm btn-default\">";
							$navi_string .= $a."</a></li>";
							$navi_string .= ($theNext < ($end - 1)) ? $separator : "";
						} else {
				...
				$navi_string .= "<li>".(($curr_pages < $all_pages) ? "<a href=\"".$_SERVER['PHP_SELF']."?".$var."=".$forward.$this->rebuild_qs($var)."\" class=\"btn btn-sm btn-default\">".$this->str_forward."</a>" : '<a class="btn btn-sm btn-default" href="javascript:{};">'.$this->str_forward.'</a>')."</li>";
			}
		}
		return $navi_string;
	}
