...
//            $update_data = array();
//            $update_data = array_copy('id,category_id,code,title,slug,short_description,long_description,adult_price,child_price,sort_order,is_active,is_feature,transpot_type,transpot_no,parking,address,language,museum,accessibiliy,pet_allowed,audio_guide,tour_guide,attraction,wildlife,park,garden,adventure,free_entry,nature,scenic,culture,food,relaxing,activity,one_way_transfer,two_way_transfer,free_and_easy,hours,promo_adult_price,promo_child_price,feature,time,date_mandatory' . $dated, $_POST);
        ...
        <div class="box-content">
            <form action="" id="product_form" name="product_form" method="post" class="form-horizontal" enctype="multipart/form-data">
                ...
                <fieldset>
                    ...
                    <div class="control-group">
                        <label class="control-label" for="sort_order">Other Services</label>
                        <div class="controls top_m">                        	
                            <input data-no-uniform="true" type="checkbox" name="museum" id="Museum" class="other_services_checkbox" <?php if(isset($museum)){ echo ($museum == '1' ? ' checked="checked"' : ''); }else{ ?>   <?php } ?>/><label class="other_services">Mesum</label>
                     ...
                     <div class="control-group">
                        ...
                        <div class="controls">                        
                                ...
                                <div class="all-inner">
                                    ...
                                    <select class="RFValues" id="leftValues" name="time[]" size="5" multiple>
...
<div class="row-fluid sortable">		
    <div class="box span12">
        <div class="box-header well" data-original-title>
            <h2><i class="icon-list"></i> Products Listing</h2>
            <div class="box-icon">
                <!--<a href="#" class="btn btn-setting btn-round"><i class="icon-cog"></i></a>-->
                <a href="#" class="btn btn-minimize btn-round"><i class="icon-chevron-up"></i></a>
                <a href="#" class="btn btn-close btn-round"><i class="icon-remove"></i></a>
        ...
        <div class="box-content">
            <table class="table table-striped table-bordered bootstrap-datatable datatable" id="table_<?php echo DB_TABLE_PREFIX . "product"; ?>">
                ...
                <tbody>
                    ...
                    $sqlquery = "SELECT product.id AS id,product.category_id AS category_id,product.code,product.title AS title,product.short_description,product.long_description,product.adult_price,product.child_price,product.image,product.sort_order,product.is_feature,product.promo_adult_price,product.promo_child_price,product.is_active,product.is_deleted,product.dated,product.last_updated,category.id AS cat_id,category.title AS category FROM product INNER JOIN category ON category.id = product.category_id where product.is_deleted ='N' ORDER BY product.sort_order ='0', product.sort_order";
                    $rows_data = mysql_query($sqlquery);
                    $product_data = mysql_num_rows($rows_data);
...
//                        foreach ($product_data as $product) {
                        while ($product = mysql_fetch_assoc($rows_data)) {   
                            extract($product);
                            $image_path = '../img/products/' . $image;
                            if ($image == "" || !file_exists($image_path))
                                $image_path = '../img/products/noimage.jpeg';
                            ...
                            <tr id="row_<?php echo $id; ?>">
                                ...
                                <td><?php echo stripcslashes(Decode($title)); ?></td>
                                <td><?php echo $code; ?></td>                        
